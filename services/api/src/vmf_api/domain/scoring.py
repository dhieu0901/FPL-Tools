...
def totw_winners(
    net_scores: dict[int, int], eligible_manager_ids: Iterable[int] | None = None
) -> set[int]:
    ...
    candidates = {
        manager_id: score for manager_id, score in net_scores.items() if manager_id in eligible
    }
    if not candidates:
        return set()
